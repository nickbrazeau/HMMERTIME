...
void MCMC::update_transition_lookup(double f, double rho, int k, int m1, int m2, Rcpp::Function getTransProbs) {
  ...
  vector< vector<double> > Evectors = rcpp_to_matrix_double(Elist["Evectors"]);
  vector< vector<double> > Esolve = rcpp_to_matrix_double(Elist["Esolve"]);
...
void MCMC::get_IBD() {
  ...
  for (int j=0; j<L; j++) {
    ...
    for (int z = 1; z <= z_max; z++){
      f_ind += IBD_mat[z][j] * z * SNP_dist[j]; // AUC -- z+1 to exclude the zero level
    }
